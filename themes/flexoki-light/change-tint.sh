...
# Flexoki Light Theme Tint Color Changer

set -e

# Current tint color (blue)
CURRENT_TINT="#205EA6"  # blue-600 (for light theme)

# Available tint colors based on Flexoki palette (light theme uses -600 variants)
...
# Available Yaru themes for reference:
# Yaru, Yaru-blue, Yaru-dark, Yaru-magenta, Yaru-olive,
# Yaru-prussiangreen, Yaru-purple, Yaru-red, Yaru-sage,
...
list_colors() {
    echo "Available Flexoki tint colors (light theme):"
    echo ""
    for color in "${!TINT_COLORS[@]}"; do
        printf "  %-10s %s\n" "$color" "${TINT_COLORS[$color]}"
    done
...
case "$1" in
    ...
    --reset)
        ...
        update_tint "#205EA6" "blue (original)"
        exit 0
        ;;
    *)
